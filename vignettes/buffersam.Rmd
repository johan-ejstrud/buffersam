...
## Strata and Elements
The _survey area_ is divided into a number of _strata_ which contains a number of _elements_. Each element is a fixed point with a surrounding area. In the West Greenland shrimp survey each element is the center point of an approximately 2 nautical miles square.
...
Elements can be placed so its surrounding area is inside two or more strata. This can be represented in the `elements` data frame by having multiple rows with the same `elementId` but a different values for `stratum`. 
